  ...
  await db.$transaction(async (prisma) => {
    if (updatedIssues.length > 0) {
      ...
      if (
        firstIssue &&
        firstIssue.sprint &&
        firstIssue.sprint.status !== "ACTIVE"
      ) {
        for (const issue of updatedIssues) {
          if (issue.status !== "TODO") {
            throw new Error(
              "Cannot change issue status when sprint is not active",
            );
          }
        }
      }
...
export async function updateIssue(issueId, data) {
  ...
  try {
    ...
    if (
      ...
      throw new Error("Cannot change issue status when sprint is not active");
    }
...
export async function getUserIssues(userId, orgId = null) {
  if (!orgId) {
